...
public class ProductManagerController implements Initializable {
   ...
   @FXML
    public void addToCart(ActionEvent event) throws IOException,SQLException{ 
      ...
      String qte=table.getSelectionModel().getSelectedItems().get(0).getQuantity();
             ...
             Lignepanier lp = new Lignepanier(id_produit,panier.getIdpan(),1);
                ...
                if(panier.getIdpan()>panier1.getIdpan())
                {
                   // System.out.println(lpService.trouve(id_produit,Main.user_id,panier.getIdpan()));
                    System.out.println(table.getSelectionModel().getSelectedItems().get(0).getQuantity());
                 if(qte.equals("0"))
                 {
                 ...
                 else
                 {
                    if(lpService.trouve(id_produit,Main.user_id,panier.getIdpan())==true )
                     { new Alert(Alert.AlertType.INFORMATION, "reajout").showAndWait();
                       panier = panierService.getCurrentPanierByUserID(Main.user_id);
                       lpService.decrement_Stock(id_produit);
                       lpService.increment_qnt(id_produit,panier.getIdpan());
                       lpService.prixupdate(panier.getIdpan(),Main.user_id);
                      
                     }
                     else
                     {   new Alert(Alert.AlertType.INFORMATION, "sucess").showAndWait();
