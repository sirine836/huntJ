...
public class userservices implements IUsers{
    ...
    public void update(User u) {
        String req = "UPDATE fos_user set username=?,username_canonical=?,email=?,password=?,enabled=? WHERE id = ?";
        PreparedStatement preparedStatement;
        try {
            preparedStatement = connection.prepareStatement(req);
            preparedStatement.setString(1, u.getNom());
   ...
   usersList.add(u);
    ...
    usersList.add(u);
     ...
     public List<User> getEmail(String email) {
      ...
      public List<User> getStatus(int status) {
        List<User> usersList = new ArrayList<>();
        String req = "SELECT * FROM fos_user where enabled =?";
        PreparedStatement preparedStatement;
        try {
            preparedStatement = connection.prepareStatement(req);
